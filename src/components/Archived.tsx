...
export const Archived = () => {
  const getArchivedLists = useBoardStore((state) => state.getArchivedLists);
  const archivedLists = getArchivedLists();

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-8">Listas Archivadas</h1>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {archivedLists.length === 0 && (
          <p className="text-gray-500 dark:text-gray-400 col-span-3 text-center py-8">
            No hay listas archivadas para mostrar
          </p>
        )}
      </div>
    </div>
